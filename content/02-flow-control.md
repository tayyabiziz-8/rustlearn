...
`if` is an expression in Rust. Both branches must produce the same type if you are going to bind the result.
...
fn main() {
    ...
    for name in &names {
        // borrowing `&names` instead of moving it so `names` is still usable after
        println!("{name}");
    }
...
`loop` runs forever until a `break`. Unlike `for` and `while`, `break` can carry a value out of a `loop`, which makes `loop` an expression.
...
When loops are nested, a bare `break` or `continue` only affects the innermost loop. Label the outer loop to target it directly.
