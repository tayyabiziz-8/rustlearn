# Smart Pointers
desc: Box, Rc, RefCell, and when each one solves a problem that a plain reference cannot.
...
**3. You want to return a trait object whose concrete type is not known at compile time.**
```rust
trait Shape {
    fn area(&self) -> f64;
}
...
> `Rc` is deliberately not `Send` and can only be used on a single thread. For multi-threaded shared ownership, use `Arc<T>` (atomic reference count) instead.
...
} // value and b drop here, count reaches 0, String is freed
```

The limitation: `Rc<T>` gives you a shared *immutable* reference. You cannot mutate through it without combining it with `RefCell`.
...
This is useful when you know your access pattern is safe but the compiler's static analysis cannot prove it. A common case is mutable state inside a callback, or a struct that needs to mutate itself while being accessed through a shared reference.
...
The most common pattern you will see: wrap `RefCell<T>` in an `Rc<T>` to get multiple owners who can each mutate the inner value.
...
`Deref` is what allows `*box_value` to work just like `*reference`. It also powers **deref coercion**: the compiler automatically inserts `*` chains so that `&Box<String>` can be passed wherever `&str` is expected.
...
} // B drops, then A drops in LIFO order (reverse of declaration)
```

> To force an early drop, use `std::mem::drop(value)`. This moves the value into `drop`, which runs the destructor immediately. Calling `value.drop()` directly is a compile error because you would be calling `Drop::drop`, which takes `&mut self` not by value, and Rust disallows calling the destructor manually to prevent a double-free.
