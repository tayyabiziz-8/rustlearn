...
fn main() {
    ...
    // Relative path also works here, since we're at the crate root:
...
The examples above use `mod front_of_house { ... }` with the body inline, but in a real project each module typically gets its own file. Writing `mod front_of_house;` (note: no body, just a semicolon) tells Rust "load this module's contents from a file":
...
```rust
// src/main.rs
mod front_of_house; // Rust looks for ./front_of_house.rs (or
                     // ./front_of_house/mod.rs in the older style)
...
`pub use` re-exports a path under the current module, so callers don't need to know your internal structure to reach it.
